...
app.get("/", function(req, res) {  
  ...
  _.forEach(posts, function(post) {
    let truncatedBody = _.truncate(post.body, {
      'length': 100, 'omission': '....'
    });
    truncatedPosts.push({ ...post, body: truncatedBody });
  });

  res.render("home", {
    homeStartingContent: homeStartingContent,
    posts: truncatedPosts, post: posts
  });
});
...
app.post("/compose", function(req, res) {
  ...
  const post = {title: req.body.postTitle, 
    ...
    posts.push(post);
    //console.log(post.title, post.body, posts);
